...
RUN_LIST="${RUN_LIST:-$ROOT/runs/$RUN_ID/scannet_MSA_pypropel_no_len_limit/run_list.tsv}"
OUT_ROOT="${OUT_ROOT:-$ROOT/results/$RUN_ID/scannet_MSA_pypropel_no_len_limit_predictions}"
LOG_ROOT="${LOG_ROOT:-$ROOT/runs/$RUN_ID/logs/scannet_MSA_pypropel_no_len_limit}"
STATUS_TSV="${STATUS_TSV:-$ROOT/results/$RUN_ID/scannet_MSA_pypropel_no_len_limit_status.tsv}"
...
idx=0
ran=0
tail -n +2 "$RUN_LIST" | while IFS=$'\t' read -r input_id pdb_path source length n_seq_ids_needed first_seq_id seq_sha256 seq_ids; do
...
echo "[OK] ScanNet MSA batch finished or reached LIMIT"
